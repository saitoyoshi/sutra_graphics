// viper:「グラディウスシリーズ」に登場するキャラクターのひとつで主役機にあたる。
(() => {
    window.isKeyDown = {};
    window.gameScore = 0;
    ...
    function initialize() {
        ...
        for(i = 0; i < EXPLOSION_MAX_COUNT; i++) {
            explosionArray[i] = new Explotion(ctx, 100.0, 15, 40.0, 1.0);
        ...
        for (i = 0; i < ENEMY_SHOT_MAX_COUNT; i++) {
            ...
            enemyShotArray[i].setExplotions(explosionArray);
        }

        for (i = 0; i < HOMING_MAX_COUNT; i++) {
            homingArray[i] = new Homing(ctx, 0, 0, 32, 32, "./image/homing_shot.png");
            homingArray[i].setTargets([viper]);
            homingArray[i].setExplotions(explosionArray);
        }

        boss = new Boss(ctx, 0, 0, 128, 128, "./image/enemy_large.png");
        ...
        for (i = 0; i < SHOT_MAX_COUNT; i++) {
            ...
            shotArray[i].setExplotions(explosionArray);
            singleShotArray[i * 2].setExplotions(explosionArray);
            singleShotArray[i * 2 + 1].setExplotions(explosionArray);
        }
    ...
    function sceneSetting() {
        ...
        scene.add("invade_boss", (time) => {
            if (scene.frame === 0) {
                boss.set(CANVAS_WIDTH / 2, CANVAS_HEIGHT / 2, 100)
                boss.setMode('invade');
            }
            if (viper.life <= 0) {
              scene.use("gameover");
        ...
        scene.add('gameover', time => {
            ...
            util.drawText('GAME OVER', x, CANVAS_HEIGHT / 2, 'red', textWidth);
            if (restart === true) {
                restart = false;
                gameScore = 0;
                viper.setComing(
    ...
    function render() {
        ...
        util.drawText(zeroPadding(gameScore, 5), 30, 50, 'green');
        scene.update();
